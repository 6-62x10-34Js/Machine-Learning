...
class BasisFunctionTransformer:
    # non functional try to make adaptive basis function transformaion
    ...
    def transform(self, X):
        if self.basis_function == 'polynomial':
            transformed_X = self.polynomial_basis(X)
        elif self.basis_function == 'circular':
            self.center = np.mean(X, axis=0)
            transformed_X = self.circular_basis(X, self.center)
        # Add more basis functions as needed
        else:
            raise ValueError(f"Invalid basis function: {self.basis_function}")

        return transformed_X

    def fit(self, X):
        if self.basis_function == 'circular':
            self.center = np.mean(X, axis=0)

    @staticmethod
    def polynomial_basis(X):
        transformed_X = np.hstack([X, X ** 2])
        return transformed_X

    @staticmethod
    def circular_basis(X, center):
        center = np.mean(X, axis=0)
        distances = np.linalg.norm(X - center, axis=1)
        transformed_X = np.expand_dims(distances, axis=1)
        return transformed_X
...
def main():
    ...
    # basis_transformer = BasisFunctionTransformer(basis_function='circular')

    x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=1)
    # x_train = basis_transformer.transform(x_train)
    learning_rate = 0.001
    n_iter = 200
